...
// 监听 defaultGroupId 变化
watch(
  () => props.defaultGroupId,
  (newVal) => {
    if (newVal && !formData.group_id) {
      formData.group_id = newVal;
    }
  },
  { immediate: true }
);
...
const fetchGroups = async () => {
  try {
    ...
    if (res.data.errcode === 0) {
      ...
      // 默认选择第一个可用的分组或传入的 defaultGroupId
      if (!formData.group_id && groupOptions.value.length > 0) {
...
const resetForm = () => {
  ...
  formData.desc = "";
  formData.basepath = "";
  formData.project_type = "private";
  formData.mock_url = "/mock";
  // 保留 group_id
};
