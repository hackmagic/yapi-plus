    ...
    <n-form ref="formRef" :model="formData" label-placement="left" label-width="120px">
      ...
      <n-card title="基本信息" :bordered="false">
        ...
        <n-form-item label="接口描述" path="desc">
          <n-input v-model:value="formData.desc" type="textarea" placeholder="接口描述" :rows="3" />
        </n-form-item>
      </n-card>
      ...
      <n-card title="请求参数" :bordered="false" style="margin-top: 16px">
        <n-tabs type="line">
          <n-tab-pane name="query" tab="Query">
            <param-table v-model="formData.req_query" type="query" @change="handleParamChange" />
          </n-tab-pane>
          ...
          <n-tab-pane name="body" tab="Body">
            ...
            <div v-if="formData.req_body_type === 'json'" style="margin-top: 12px">
              <ace-editor v-model="formData.req_body_other" mode="json" height="300px" />
            </div>
            <param-table
      ...
      <n-card title="返回数据" :bordered="false" style="margin-top: 16px">
        <ace-editor v-model="formData.res_body" mode="json" height="400px" />
      </n-card>

      <!-- 操作按钮 -->
      <div style="margin-top: 24px">
        <n-button type="primary" @click="handleSubmit" :loading="submitting"> 保存 </n-button>
        <n-button @click="handleCancel" style="margin-left: 12px"> 取消 </n-button>
      </div>
    </n-form>
  </div>
</template>
