    ...
    <n-modal v-model:show="showAddMember" preset="card" title="添加成员" style="width: 500px;">
      <n-form @submit.prevent="handleAddMember">
        ...
        <n-space justify="end">
          ...
          <n-button type="primary" @click="handleAddMember" :loading="adding">
            添加
          </n-button>
        </n-space>
      </n-form>
...
const columns = [
  ...
  },
  {
    ...
    render: (row) => {
      ...
      return h(NTag, { type: roleMap[row.role] || 'default' }, {
        default: () => row.role
      })
    }
  },
    ...
    render: (row) => h(NSpace, {}, {
      default: () => [
        h(NPopconfirm, {
          ...
          trigger: () => h(NButton, { size: 'small', type: 'error', text: true }, {
            default: () => '移除'
          }),
          default: () => '确定要移除此成员吗？'
        })
