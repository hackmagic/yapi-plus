...
- 测试基线：`npm test` 可通过（Ava）
- 进度文档：`TASK_PLAN.md` 是唯一真实进度源；迁移类文档仅作历史参考
...
> Windows 使用 `start-dev.bat` 时：
> - 若 `mongod` 已在系统 `PATH` 中，脚本会自动拉起 MongoDB；
> - 若未在 `PATH`，请先设置 `YAPI_MONGOD_PATH`（可选再设置 `YAPI_MONGO_DBPATH`）后再执行。
>
> ```bat
...
5. **数据库连接失败**：检查 MongoDB 是否运行，连接字符串是否正确
   - Windows 下运行 `start-dev.bat` 失败时，优先确认：
     - `mongod` 是否已安装且可执行（`where mongod`）
     - 是否正确设置 `YAPI_MONGOD_PATH`
     - 若指定了数据目录，`YAPI_MONGO_DBPATH` 是否存在且有写权限
